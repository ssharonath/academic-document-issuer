...
async function renderDocumentBlob(type = "png", ref = "", student = state.selectedStudent, template = state.template) {
  ...
  if (template.background) {
    await drawImageCover(ctx, template.background, 0, 0, size.width, size.height);
  }
...
function drawTableCellText(ctx, text, x, y, w, h, style, isHeader = false) {
  const padX = style.mode === "tabbed" ? 2 : 5;
  ...
  if (style.mode === "tabbed") {
    ctx.fillText(String(text), x + padX, y + padY, maxWidth);
    ctx.restore();
    return;
  }
